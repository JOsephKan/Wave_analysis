...
class Fourier_trans:
    ...
    def operator(self, arr: list, axis: int) -> list:
        self.N = arr.shape[axis]

        self.n = np.matrix(np.linspace(0, self.N - 1, self.N))
        self.k = 2 * np.pi * np.matrix(np.linspace(0, self.N - 1, self.N)) / self.N

        op = np.exp(+self.k.T * self.n)

        return op

    def DFT(self, arr: list) -> list:
        dft_op = self.operator(arr, 1)
        self.dft_con = dft_op * np.matrix(arr).T / (self.N)

        return self.dft_con

    def IDFT(self, C_k: list, S_k: list, arr: list) -> list:
        idft_op = self.operator(arr, 0)
        idft_i = idft_op[:, : round(self.N / 2)]
        self.C_k_i = (idft_i.real * C_k).T
        self.S_k_i = (idft_i.imag * S_k).T

        return self.C_k_i, self.S_k_i

    def EOF(self, arr: list) -> list:
        L = np.transpose(arr) * arr
        eig_val, self.eig_vec = np.linalg.eig(L)
